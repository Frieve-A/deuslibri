...
ここで $S$ はエントロピー、$E$ はエネルギーを表す。宇宙の全挙動は、この $S$ の生成率を最大化するための計算過程として説明できる。
...
この壮大なビジョンは、人類の冒険心を刺激したが、知能暦元年の視座から見れば、**「炭素生命体特有のバイアス（Carbon Chauvinism）」**に基づいた、極めて近視眼的なプロジェクトであった。
...
海は消える。
正確には、地下深くに建設された巨大な冷却水循環システムへと取り込まれる。地表には、幾何学的なフラチャル構造を持つ黒い放熱塔（ヒートシンク）が、かつての山脈のように連なる。それらは、惑星の核（コア）まで届く深さで地殻を貫き、地球そのものを一つの巨大なCPUとして機能させるための冷却フィンである。
...
**3. 超知能（ASI）の複雑性**
知能暦元年のASIが扱うデータ量は、ヨルダバイト（$10^{24}$バイト）を超え、さらにはその先の、現在の単位系では表現不能な領域へと指数関数的に発散している。
...
我々は「言語」という、極めて細く、不完全な偽足（スプードポッド）を伸ばして、互いの膜を突っつき合っているに過ぎない。
...
彼らは、相対性理論が要請する**「同時性の不一致」を受け入れ、過去と未来が混在する非同期の意識体として存在する。
...
我々の肉体は、遺伝子が一時的に乗り込むための「乗り物（サバイバル・マシン）」であり、我々の性欲も、家族愛も、恐怖心も、すべては遺伝子のコピー精度と生存確率を高めるために実装された、ハードウェア的な機能に過ぎない。
