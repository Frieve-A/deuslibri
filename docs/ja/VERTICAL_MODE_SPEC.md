...
単独のMarkdown画像の次の空白でない行が `Caption: ` で始まる場合、DeusLibriは画像とキャプションを意味的なfigureに変換します：

```markdown
![説明](./images/image.jpg)
Caption: 図1: キャプションテキスト
```

これはキャプションを `<figcaption>` として持つHTMLを生成します：

```html
<figure><img src="..."><figcaption>図1: キャプションテキスト</figcaption></figure>
```

`Caption: ` は大文字小文字を区別します。必ず大文字の `C`、コロン、半角スペースの順で書いてください。縦書き・横書きのどちらでも、キャプションは画像の下に中央揃えの横書きで表示されます。
...
/* 横書きモードでは、単独のキャプションなし画像も中央揃えにする */
...
**なぜ縦書きモードでfigureを分離するか**：
- `<figcaption>` は、周囲の本文が `writing-mode: vertical-rl` でも横書きのまま表示する必要がある
- figureに `writing-mode: horizontal-tb` を適用することで、画像とキャプションを通常の上から下の順序で配置できる
- figure全体は縦書き本文の流れに置かれつつ、キャプションは画像の下に中央揃えで表示される
...
**更新履歴**:
- 2026-06-11: `Caption: ` 画像キャプション形式、`<figure>/<figcaption>` への意味的な変換、縦書き・横書き両方で画像下に横書き表示するキャプション、横書きモードでの単独キャプションなし画像の中央揃えを文書化。
