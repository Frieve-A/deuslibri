...
unlisted: true              # カタログとサイトマップには表示せず、直接URLでは読めるようにする
aiUsage: ai_generated       # AI利用の開示バッジを表示する
```
...
| `unlisted` | - | 公開一覧には表示せず、書籍の直接URLでは読めるようにする | `true` |
| `aiUsage` | - | 書籍詳細にAI利用の開示バッジを表示する | `ai_generated` |

### 限定公開（カタログ非表示）

書籍を公開しつつ、公開カタログには表示したくない場合は、`metadata.yml` に `unlisted: true` を追加します。

```yaml
unlisted: true
```

`unlisted: true` の書籍は、トップページ、カタログページ、生成されるサイトマップから除外されます。一方で書籍ページ自体は生成されるため、直接URLを知っている人は閲覧できます。

```text
/book/{id}/{language}
```

これはアクセス制御ではありません。非公開情報や機密情報の保護には使わないでください。値はブール値の `true` にしてください。文字列の `"true"` は有効扱いされません。

### AI利用の開示

生成AIにより生成・翻訳された文章や画像を著者が編集・改稿した箇所を含む書籍では、`metadata.yml` に `aiUsage: ai_generated` を追加します。

```yaml
aiUsage: ai_generated
```

これにより、書籍詳細モーダルにAI利用の開示バッジが表示されます。バッジでは、著者がプロンプト設計、出力の選択、構成、加筆修正、検証、最終編集を行い、書籍全体の責任を負うことを説明します。

有効な値はブール値の `true` または文字列の `ai_generated` です。新しい書籍では、意図が明確な `ai_generated` を推奨します。
...
表はGitHub Flavored Markdownの表記を使用できます。DeusLibriでは、表の横スクロールバーを出さず、表示領域に収まるようにセル内で自動改行します。
...
2行目の区切り行（`---` や `----------`）は、列ごとの改行制御にも使われます。区切り行のハイフン幅が、その列に含まれる各セルの文字幅より長い列は、短いラベル列として扱われ、改行されにくくなります。上の例では1列目の `----------` が `項目`、`診断問い`、`誤用`、`修正方向` より長いため、1列目は改行を抑制します。一方、2列目の `----` は本文より短いため、内容は表示幅に合わせて折り返されます。

短い分類・項目名の列は区切り行を長めにし、長文を入れる列は短い区切り行にしてください。raw HTMLの `<table>` ではこの自動判定は適用されません。
...
```markdown
![駅前の風景](./images/illustration-01.jpg)
Caption: 図1: 事件が起きた駅前の様子
```

キャプション行は、必ず大文字の `C` で始まる `Caption: `（コロンの後に半角スペース）から書き始めてください。画像の次の空白でない行が `Caption: ` で始まる場合、その行は直前の画像の `<figcaption>` として表示されます。キャプションは縦書き・横書きのどちらでも、画像の下に中央揃えの横書きで表示されます。
...
- **ファイルサイズ**: 各500KB以下推奨
- **キャプション**: 表示キャプションが必要な場合は、画像の後に `Caption: ` 行を置きます
...
- [物語創造の公理](../../content/books/2026-06/story-creation-axioms/)
- [フィジカルAI : Ground Truth](../../content/books/2026-01/physical-ai/)
- [オーディオの終焉と再生](../../content/books/2025-12/the-end-and-rebirth-of-audio/)
